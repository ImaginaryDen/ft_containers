...
namespace ft{
	...
	public:
		...
		friend typename vector::template RandomAccessIterator<A>::difference_type
				operator-(const vector::template RandomAccessIterator<A> & lhs,
							const vector::template RandomAccessIterator<B> & rhs){
			return &(*lhs) - &(*rhs);
		}
		...
		template<typename L>
		friend vector::template RandomAccessIterator<L>
				operator+(const typename vector::template RandomAccessIterator<L>::difference_type & a,
	...
	public:
		...
		vector (InputIterator first, InputIterator last,
			...
			for (pointer it = _first; first != last; ++first, ++it)
		...
		iterator insert (iterator position, const value_type& val){
			...
			if (_size == _capacity){
				_capacity = std::max(_capacity * 2, 1);
		...
		void insert (iterator position, size_type n, const value_type& val){
			...
			if (_size + n > _capacity){
				pointer new_arr = _allocator.allocate(std::max(_capacity * 2, _size + n));
				std::uninitialized_copy(begin(), position, iterator(new_arr));
				for (size_type i = 0; i < n; i++)
